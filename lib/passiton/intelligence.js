...
export function validateAssessment(raw){
  ...
  const claims=raw.claims.slice(0,6).map(c=>{
    const s=source.sections.find(s=>s.id===c.sectionId);
...
export async function assess({question,language,text,mode='contribute'},options={}){
  ...
  try{response=await (options.fetch||fetch)(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,{
    ...
    body:JSON.stringify({systemInstruction:{parts:[{text:instruction}]},contents:[{role:'user',parts:[{text:JSON.stringify({mode,question,language:languages[language],volunteerText:text,source})}]}],generationConfig:{thinkingConfig:{thinkingLevel:'low'},temperature:0.1,maxOutputTokens:2600,responseMimeType:'application/json'}})
  ...
  try{const body=await response.json();const raw=JSON.parse(body.candidates?.[0]?.content?.parts?.filter(p=>p.text&&!p.thought).map(p=>p.text).join('')||'{}');return validateAssessment(raw);}catch{throw Object.assign(new Error('The source check did not pass citation validation. Your draft is preserved; retry.'),{status:502});}
}
