...
function authenticated(token,kind){
  ...
  if(parsed.kind!==kind||!Number.isFinite(parsed.exp))throw invalid();return parsed;
}

export function verify(token,kind){
  const parsed=authenticated(token,kind);
  if(parsed.exp<=Date.now()/1000)throw Object.assign(new Error('This review evidence is invalid or expired. Check the contribution again.'),{status:400});
  return parsed.data;
}
// Only for records already published to the server-owned warehouse. Publication
// outlives action authorization; never use this to accept a token from a request.
// The caller must check the current source version and requested topic/language.
export function verifyStoredReview(token){return authenticated(token,'review').data;}
