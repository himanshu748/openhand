...
export async function recordEvent(event){
  ...
  try{
    const result=await sql('MERGE INTO PASSITON_EVENTS t USING (SELECT ? AS id, PARSE_JSON(?) AS payload) s ON t.ID=s.id WHEN NOT MATCHED THEN INSERT (ID, PAYLOAD, CREATED_AT) VALUES (s.id, s.payload, CURRENT_TIMESTAMP())',[event.id,JSON.stringify({...event,sourceVersion})]);
...
export async function coverage(){
  ...
  try{const result=await sql(COVERAGE_SQL,[sourceVersion,sourceVersion]);return {source:'snowflake',statementHandle:result.statementHandle,rows:result.rows.map(r=>({...r,requests:Number(r.requests),reviewed:Number(r.reviewed),unmet_requests:Number(r.unmet_requests)}))};}
