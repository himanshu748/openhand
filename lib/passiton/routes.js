...
import {source,sourceVersion,questions,languages,getQuestion,getLanguage,bounty} from './catalog.js';
import {issue,verify,digest} from './proofs.js';
...
import {coverage,recordEvent,warehouseConfigured,sql,COVERAGE_SQL} from './warehouse.js';
...
export function createPassitonRouter(overrides={}){
  const deps={assess,coverage,recordEvent,findPayment,verifyPayment,transcribe,speakChecked,...overrides};
  ...
  const session=req=>verify(req.get('X-Passiton-Session'),'session').id;
  ...
  router.get('/catalog',(_req,res)=>res.json({source,sourceVersion,questions,languages,bounty,integrations:{gemini:geminiConfigured()?'configured':'not configured',elevenlabs:voiceConfigured()?'configured':'not configured',transcription:transcriptionConfigured()?'configured':'not configured',snowflake:warehouseConfigured()?'configured':'not configured',solana:'devnet'},scope:'Fictional scenario. Human review is a demo role, not authenticated independent review. Test SOL has no monetary value.'}));
  ...
  router.get('/coverage/sql',(_req,res)=>res.json({sql:COVERAGE_SQL,description:'Counts distinct demo sessions asking each question, joins human-reviewed contributions by language and current source version, and ranks unmet demand.'}));
  router.get('/knowledge/:questionId/:language',route(async(req,res)=>{
    ...
    if(!warehouseConfigured())return res.json({answers:[],source:'local'});
    try{const result=await sql("SELECT PAYLOAD:reviewToken::STRING token FROM PASSITON_EVENTS WHERE PAYLOAD:type::STRING='review' AND PAYLOAD:questionId::STRING=? AND PAYLOAD:language::STRING=? AND PAYLOAD:sourceVersion::STRING=? ORDER BY CREATED_AT DESC LIMIT 5",[req.params.questionId,req.params.language,sourceVersion]);
      const answers=[];for(const row of result.rows){try{const data=verify(row.token,'review');if(data.sourceVersion===sourceVersion&&data.questionId===req.params.questionId&&data.language===req.params.language)answers.push({reviewToken:row.token,answer:data});}catch{}}
  ...
  router.post('/question',route(async(req,res)=>{
    ...
    const storage=await deps.recordEvent({id:`question:${sid}:${q.id}:${language}`,type:'question',sessionId:sid,questionId:q.id,language});
  ...
  router.post('/speak',route(async(req,res)=>{
    ...
    if(req.body.kind==='opening'){
      ...
      if(kind==='review')text=data.claims.map(c=>c.text).join(' ');
  ...
  router.use((err,_req,res,_next)=>{res.status(err.status||502).json({error:err.status?err.message:'The service could not finish this step. Your draft is preserved; please retry.'});});
  return router;
}
