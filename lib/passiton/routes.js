...
export function createPassitonRouter(overrides={}){
  const deps={assess,coverage,recordEvent,findPayment,verifyPayment,transcribe,speakChecked,warehouseConfigured,sql,...overrides};
  ...
  router.post('/speak',route(async(req,res)=>{
    ...
    if(!['opening','review','reading','draft','check'].includes(req.body.kind))throw fail('Choose a supported spoken guidance type.');
